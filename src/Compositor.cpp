...
void Compositor::desassociar(list<Nota> &listaDeNotas){
    ...
    else
    {
        for(auto nota : listaDeNotas)
        {
            ...
            alturasSemOitavas.push_back(nota.pegueAltura()+nota.pegueAcidente1()+nota.pegueAcidente2());
...
{ 
    alturas.erase(alturas.begin(), alturas.end()); 
    alturasSemOitavas.erase( alturasSemOitavas.begin(), alturasSemOitavas.end() );
}
